...
class CategoryAlreadyExistsError(ValidationError):
    ...
    def __init__(self):
        super().__init__({"name": _("You already have a category with this name and type.")}, code=self.default_code)

class InvalidCategoryError(ValidationError):
    default_code = "invalid_category"
    def __init__(self):
        super().__init__({"category_id": _("Invalid category.")}, code=self.default_code)
...
class MissingInstallmentsTotalError(ValidationError):
    ...
    def __init__(self):
        super().__init__(
            {"installment_total": _("Necessary to inform the number of installments when the installment value is fixed.")},
...
class InvalidRecurrenceUpdateError(ValidationError):
    ...
    default_detail = _("To update recurrence, you must choose either all following transactions or only this one.")
...
class CategoryHasTransactionsError(ValidationError):
    ...
    def __init__(self):
        super().__init__({"error": _("It is not possible to delete a category that has associated transactions.")}, code=self.default_code)

class AccountHasTransactionsError(ValidationError):
    default_code = "account_has_transactions"
    def __init__(self):
        super().__init__({"error": _("It is not possible to delete an account that has transactions. Archive it or delete the transactions first.")}, code=self.default_code)

class NotInPaymentPlanError(APIException):
    status_code = status.HTTP_400_BAD_REQUEST
    default_code = "not_in_payment_plan"
    def __init__(self):
        super().__init__({"error": _("This transaction is not part of a payment plan.")}, code=self.default_code)

class InvalidDateUpdateError(ValidationError):
    default_code = "invalid_date_update"
    default_detail = _("For updating this or following transactions, date must not be changed.")
