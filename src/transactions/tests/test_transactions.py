...
class TransactionRegressionsTest(APITestCase):
    ...
    def test_delete_series_regression(self):
        ...
        url = reverse("transactions:transactions-delete-series", args=[transactions[0].id])
        
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        
        self.assertFalse(Transaction.objects.filter(installment_group_id=group_id).exists())
